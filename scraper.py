...
def extract_next_links(url, resp, soupt):
    ...
    no_stop_words = cust_tokenize(soupt) #call tokenize function to get all the valuable words on the webpage, as a list
    ...
    if (len(no_stop_words) < 100 and len(unparsed_href_list) < 2) or (len(resp.raw_response.content) < 500): #checking if the page is a low value information page
...
def is_valid(url):
    ...
    try:
        ...
        #calendar stuff
        #or ('event' in url) or ('calendar' in url) or ('events' in url) or 
